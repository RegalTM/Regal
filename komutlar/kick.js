...
Etiketliyorsan Bu HMediaayı Alıyorsan O Üyenin Görebildiği Bir Kanalda Banlamayı Denemelisin`)
    }
    
    if (!sebep) {
      return message.reply(`HMediaa: Sunucudan banlancak kişiyi veya ban sebebini yazmadın!`)
    }
    if (banl == db.fetch(`banP_${message.author.id}`)) {
    return message.channel.send(`<@${message.author.id}> **HMediaa:** Ban limitin doldu!`)  
    } else {
    ...
    return message.reply("Ban Mediaabilmek İçin Sunucu Sahibinin Ayarladığı Role Sahip Olmalısınız.")
  } 
